...
```
fun login(phone: String, userBasePhone: String?, name: String?, deviceId: String?, loyaltyId: String?) {
    lifecycleScope.launch {
        try {
            wallet.login(phone, userBasePhone, name, deviceId, loyaltyId)
        } catch(e: Exception) {
            Log.d(e)
        }
    }
...
##### Получение данных по профилю пользователя.
...
##### Получение данных по профилю пользователя, детализация.
...
`login(userPhone: String, userBasePhone: String?, userName: String, deviceId: String?, loyaltyId: String?): Int`
**Параметры**
...
`getBankLinkageStatus(linkId: Int): BankLinkageResult`
**Параметры**
...
| Тип      | Опциональный | Описание |
| ----------- | ----------- | ----------- |
| BankLinkageResult | Нет| Результат привязки банка. |
