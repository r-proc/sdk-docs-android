...
`getUserExperiment(phone: String): UserExperiments`
**Возвращает**
| Тип      | Опциональный | Описание |
| ----------- | ----------- | ----------- |
...
`login(userPhone: String, userName: String, uid: String?): Int`
**Параметры**
...
| banks | List<Bank> | нет  | 	Список привязанных к кошельку счетов |
...
| isLinked | Boolean | нет (false по умолчанию) | Состояние привязки счета. Если банк присутствует в списке, но его статус isLinked == false, это означает что счет в процессе привязки.   |
| isDefault | Boolean | нет (false по умолчанию) | Счет банка выбранный основным. С этого счета будут сниматься средства при платежах кошельком.  |
